...
ICE 连接完成、收到轨道不代表已经收到可显示的视频帧。先看眼镜的“采集”尺寸和帧率，再看“发送”尺寸与视频码率，最后看浏览器的接收统计，按这三个位置定位数据停在哪里。两端显示的尺寸可以因 WebRTC 适配而不同。

只选音频时，浏览器保留“未发送视频”的提示是正常现象。纯视频被浏览器拦截自动播放时，点击“播放视频”恢复；音频轨道到达后按钮显示“开启声音/静音”。连续 3 秒无新数据会回到等待状态，数据恢复后重新显示接收状态。

把源码接入自己的 App 后遇到模块路径或类找不到，请按 [源码组件接入](source-integration.md) 核对目录、settings 注册与 App 依赖，不要只复制一行依赖声明。
